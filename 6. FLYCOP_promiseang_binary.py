...
def make_df_and_graph(strains, metabolites, comets, max_cycles):
    ...
    for d in metabolites:
        ...
        while j < (max_cycles): #For each cycle
            ...
            j+=1
    df_biomass.columns=columns
    np.savetxt(r'biomass_binary_vs_'+file_name+'_template.txt', df_biomass.values, fmt='%s',delimiter='\t',header='\t'.join(columns)) #The data is saved
    ...
    for text, font in zip(legend.get_texts(), [font_italic] * 2 + [font_normal] * (len_lines-2)):
        text.set_fontproperties(font)
    
    #saving the figure to a pdf
    plt.savefig('biomass_binary_vs_'+file_name+'_template_plot.pdf')
...
def FLYCOP_space():
    ...
    o1 = Constant("output_file",value="promiseang_bottom_up_binary.csv")
    
    cs.add_hyperparameters([e1,e2,b1,b2,o1])
    ...
    scenario = Scenario(
        cs,
        deterministic = True,
        output_directory=Path("./PROMISEANG_FLYCOP_results_binary"),
...
def FLYCOP_schema(config: Configuration, seed: int = 0) -> float:
    ...
    # Layout
    layout = c.layout()
    create_complete_layout([bacteria_1_cobra, bacteria_2_cobra],[biomass1, biomass2], medio_general, layout)

    # Metabolites to analyse (available sugars which can be consumed by the bacteria)
    metabolites = carbon_sources([bacteria_1_cobra, bacteria_2_cobra])
    ...
    comets.total_biomass.to_csv('Total_biomass_binary_log_template.txt',sep='\t',index=False)
    ...
    final_biomasses = [final_biomass1, final_biomass2]
    ...
    header_line="Configuration\tfitness\t"+"Sugars_consumed\t"+"\tBiomass1\tBiomass2\tEndCycle"
    print(header_line)
    output_line=configuration+'\t'+str(fitness)+'\t'+sugars_consumed+'\t'+str(final_biomass1)+'\t'+str(final_biomass2)+'\t'+str(endCycle)
    print(output_line)
